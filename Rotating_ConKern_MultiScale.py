...
class RotatingConvCreator2D(Layer):
	def _init_(self, **kwargs):
		super(RotatingConvCreator2D, self).__init__(**kwargs)
	
	def build(self, input_shape):
		super(RotatingConvCreator2D, self).build(input_shape)
	...
	def call(self, x):
		...
		M1 = []
		for l in L_:
			L1.append([K.expand_dims(K.expand_dims(t, axis=1), axis=1) for t in tf.unstack(l, axis=1)])
			M1.append([K.expand_dims(K.expand_dims(t, axis=1), axis=1) for t in tf.unstack(l, axis=1)[::-1]])
		L.extend([L1, M1])
		# Kernel and its mirror rotated by +90 degrees
		L2, M2 = [], []
		for i in range(k):
			L2.append([L1[k-1-j][i] for j in range(k)])
			M2.append([M1[k-1-j][i] for j in range(k)])
		L.extend([L2, M2])
		# Kernel, mirror rotated by +180 degrees
		L2, M2 = [], []
		for i in range(k):
			L2.append([L1[k-1-i][k-1-j] for j in range(k)])
			M2.append([M1[k-1-i][k-1-j] for j in range(k)])
		L.extend([L2, M2])
		# Kernel, mirror rotated by +270 degrees
		L2, M2 = [], []
		for i in range(k):
			L2.append([L1[j][k-1-i] for j in range(k)])
			M2.append([M1[j][k-1-i] for j in range(k)])
		L.extend([L2, M2])
		
		# Kernels rotated by 45/-45 etc
		Lq = []
		# Kernel, mirror rotated by +45 degrees
		L2, M2 = [], []
		for i in range(k):
			l = []
			l.extend([tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(k-1-i)])
			for j in range(i+1):
				l.extend([L1[i-j][j], tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4]))])
			l = l[:-1] + [tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(k-1-i)]
			L2.append(l)
			m = []
			m.extend([tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(k-1-i)])
			for j in range(i+1):
				m.extend([M1[i-j][j], tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4]))])
			m = m[:-1] + [tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(k-1-i)]
			M2.append(m)
		for i in range(k-1):
			l = []
			l.extend([tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(i+1)])
			for j in reversed(range(i+1, k)):
				l.extend([L1[j][k-j], tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4]))])
			l = l[:-1] + [tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(i+1)]
			L2.append(l)
			m = []
			m.extend([tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(i+1)])
			for j in reversed(range(i+1, k)):
				m.extend([M1[j][k-j], tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4]))])
			m = m[:-1] + [tf.zeros(shape=(x.shape[0], 1, 1, x.shape[3], x.shape[4])) for j in range(i+1)]
			M2.append(m)
		Lq.extend([L2, M2])
		# Kernel, mirror rotated by +135 degrees
		L1, L2 = L[2], L[3]
		L2, M2 = [], []
		for i in range(k):
			l = []
		...
		for i in range(k-1):
			...
			M2.append(m)
		Lq.extend([L2, M2])
		# Kernel, mirror rotated by +225 degrees
		L1, L2 = L[4], L[5]
		L2, M2 = [], []
		for i in range(k):
			l = []
		...
		for i in range(k-1):
			...
			M2.append(m)
		Lq.extend([L2, M2])
		# Kernel, mirror rotated by +315 degrees
		L1, L2 = L[6], L[7]
		L2, M2 = [], []
		for i in range(k):
			l = []
		...
		kerns = []
		for l in L:
			rows = [tf.concat(row, axis=2) for row in l]
