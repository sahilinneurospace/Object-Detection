	...
	def __init__(self, img_rows=96, img_cols=128, img_channels=3, kernel_sizes=None, num_layers=5, output_layers=[0, 1, 2, 3, 4], batch_size=50, num_classes=10):
		...
		for i in range(self.num_layers):
			...
			if i in self.output_layers:
				...
				Y = Conv2D(int(frames/2), 1, activation='relu', padding='same')(Y)
				weights2 = Dense(int(frames/2), activation='sigmoid')(C)
				weights2 = Reshape((rows,cols,int(frames/2)))(RepeatVector(rows*cols)(weights2))
				Y = Multiply()([Y, weights2])
				Y = Conv2D(1, 1, activation='relu', padding='same')(Y)
