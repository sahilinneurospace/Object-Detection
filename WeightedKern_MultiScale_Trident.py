	...
	def __init__(self, img_rows=96, img_cols=128, img_channels=3, kernel_sizes=None, num_layers=5, output_layers=[0, 1, 2, 3, 4], batch_size=50, num_classes=10):
		...
		for i in range(self.num_layers):
			...
			if i in self.output_layers:
				...
				weights1_1 = Dense(frames, activation='sigmoid')(C)
				weights1_1 = Reshape((rows,cols,frames))(RepeatVector(rows*cols)(weights1_1))
				Y1 = Multiply()([X, weights1_1])
				Y1 = Conv2D(int(frames/2), 1, activation='relu', padding='same')(Y1)
				weights1_2 = Dense(int(frames/2), activation='sigmoid')(C)
				weights1_2 = Reshape((rows,cols,int(frames/2)))(RepeatVector(rows*cols)(weights1_2))
				Y1 = Multiply()([Y1, weights1_2])
				Y1 = Conv2D(1, 1, activation='relu', padding='same')(Y1)
				
				weights2_1 = Dense(frames, activation='sigmoid')(C)
				weights2_1 = Reshape((rows,cols,frames))(RepeatVector(rows*cols)(weights2_1))
				Y2 = Multiply()([X, weights2_1])
				Y2 = Conv2D(int(frames/2), 2, activation='relu', padding='same')(Y2)
				weights2_2 = Dense(int(frames/2), activation='sigmoid')(C)
				weights2_2 = Reshape((rows,cols,int(frames/2)))(RepeatVector(rows*cols)(weights2_2))
				Y2 = Multiply()([Y2, weights2_2])
				Y2 = Conv2D(1, 1, activation='relu', padding='same')(Y2)
				Y2 = MaxAssigner2D(2)(Y2)
				
				weights3_1 = Dense(frames, activation='sigmoid')(C)
				weights3_1 = Reshape((rows,cols,frames))(RepeatVector(rows*cols)(weights3_1))
				Y3 = Multiply()([X, weights3_1])
				Y3 = Conv2D(int(frames/2), 3, activation='relu', padding='same')(Y3)
				weights3_2 = Dense(int(frames/2), activation='sigmoid')(C)
				weights3_2 = Reshape((rows,cols,int(frames/2)))(RepeatVector(rows*cols)(weights3_2))
				Y3 = Multiply()([Y3, weights3_2])
				Y3 = Conv2D(1, 1, activation='relu', padding='same')(Y3)
				Y3 = MaxAssigner2D(2)(Y3)
